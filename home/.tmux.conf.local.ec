...
# MY NOTES:
# - Local template is starting point per env. Adds this message, code to
#   include my (shared) .tmux.conf.user, and some basic status bar config
# - Variables only come from local file - check notes in .tmux.conf.user if any
#   need added.
# - Suggested changes: first line of status bar will always be in local file
...
# mystuff defines/uses .tmux.conf.local for truly local (env specific) settings
# support an common conf file for my general settings

# override edit configuration bind with both files
bind e new-window -n "#{TMUX_CONF_LOCAL}" -e "EDITOR=$EDITOR" sh -c 'case "${EDITOR:-vim}" in *vim) ${EDITOR:-vim} -c ":set syntax=tmux" -p "$TMUX_CONF_LOCAL" "$TMUX_CONF_USER";; *) "$EDITOR" "$TMUX_CONF_LOCAL" "$TMUX_CONF_USER";; esac && "$TMUX_PROGRAM" ${TMUX_SOCKET:+-S "$TMUX_SOCKET"} source "$TMUX_CONF" \; display "$TMUX_CONF_LOCAL sourced"'

%if #{==:#{TMUX_CONF_USER},}
  run '"$TMUX_PROGRAM" set-environment -g TMUX_CONF_USER "$TMUX_CONF.user"'
%endif

run '"$TMUX_PROGRAM" source "$TMUX_CONF_USER"'
